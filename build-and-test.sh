...
docker build \
  --platform "${PLATFORM}" \
  -t "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}" \
...
if [[ $CURRENT_ARCH == $NATIVE_ARCH && $VARIANT == apache* ]]; then
  ...
  DOCKER_CID=`docker run --platform "${PLATFORM}" --rm -e MYVAR=foo -p "81:80" -d -v "$(pwd)":/var/www/html -e APACHE_DOCUMENT_ROOT=tests "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}"`
  # Let's wait for Apache to start
  sleep 5
  ...
  DOCKER_CID=`docker run --platform "${PLATFORM}" --rm -e MYVAR=foo -p "81:80" -d -v "$(pwd)":/var/www/foo -e APACHE_DOCUMENT_ROOT=/var/www/foo/tests "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}"`
  # Let's wait for Apache to start
  sleep 5
  RESULT=`curl http://localhost:81/test.php`
  [[ "$RESULT" == "foo" ]]
  ...
  DOCKER_CID=`docker run --platform "${PLATFORM}" --rm -p "81:80" -d -v "$(pwd)"/tests/testHtAccess:/foo -e APACHE_DOCUMENT_ROOT=/foo "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}"`
  # Let's wait for Apache to start
  sleep 5
  ...
  DOCKER_CID=`docker run --platform "${PLATFORM}" --rm -e MYVAR=foo -p "81:80" -d -v "$(pwd)":/var/www/html -e PHP_INI_MEMORY_LIMIT=2G "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}"`
  # Let's wait for Apache to start
  sleep 5
...
if [[ $VARIANT == fpm* ]]; then
  ...
  DOCKER_CID=`docker run --platform "${PLATFORM}" --rm -p "9000:9000" -d -v "$(pwd)":/var/www/html "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}"`
...
set +e
RESULT=`docker run --platform "${PLATFORM}" --rm -e CRON_SCHEDULE_1="* * * * * * *" -e CRON_COMMAND_1="(>&1 echo "foobar")" "${OWNER}/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}-${CURRENT_ARCH}" sleep 1 2>&1 | grep -o 'Cron is not available in this image'`
set -e
...
docker build \
  --platform "${PLATFORM}" \
  -t "${OWNER}/php:${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}-${CURRENT_ARCH}" \
  --build-arg PHP_VERSION="${PHP_VERSION}" \
  --build-arg GLOBAL_VERSION="${BRANCH}" \
  --build-arg OWNER="${OWNER}" \
...
docker run --platform "${PLATFORM}" --rm -e PHP_EXTENSION_XDEBUG=1 "${OWNER}/php:${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}-${CURRENT_ARCH}" php -i | grep xdebug.mode| grep "debug"
...
docker build \
  --platform "${PLATFORM}" \
  -t "${OWNER}/php:${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}-node24-${CURRENT_ARCH}" \
...
docker build \
  --platform "${PLATFORM}" \
  -t "${OWNER}/php:${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}-node26-${CURRENT_ARCH}" \
