...
test-quick: _test-prerequisites ## Test 8.0 and 8.1 quickly
	PHP_VERSION=8.0 BRANCH=v5 VARIANT=cli ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	PHP_VERSION=8.1 BRANCH=v5 VARIANT=cli ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	notify-send -u critical "Tests passed with success"

test-8.1: _test-prerequisites ## Test php8.1 build only
	docker pull ubuntu:jammy
	PHP_VERSION=8.1 BRANCH=v5 VARIANT=cli ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	PHP_VERSION=8.1 BRANCH=v5 VARIANT=apache ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	notify-send -u critical "Tests passed with success"

test-8.0: _test-prerequisites ## Test php8.0 build only
	docker pull ubuntu:jammy
	PHP_VERSION=8.0 BRANCH=v5 VARIANT=cli ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	PHP_VERSION=8.0 BRANCH=v5 VARIANT=apache ./build-and-test.sh || (notify-send -u critical "Tests failed" && exit 1)
	notify-send -u critical "Tests passed with success"
