...
test-8.0: _test-prerequisites ## Test php8.0 build only
	PHP_VERSION=8.0 BRANCH=v6 VARIANT=cli OWNER="qonstrukt" PLATFORM="linux/`uname -p`64" ./build-and-test.sh || (echo "Tests failed" && exit 1)
	PHP_VERSION=8.0 BRANCH=v6 VARIANT=apache OWNER="qonstrukt" PLATFORM="linux/`uname -p`64" ./build-and-test.sh || (echo "Tests failed" && exit 1)
	echo "Tests passed with success"
