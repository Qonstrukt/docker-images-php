...
if [ -n "$EXTENSION" ]; then
  ...
  if apt search --names-only "php${PHP_VERSION}-$PACKAGE_NAME" | grep "php${PHP_VERSION}-$PACKAGE_NAME"; then
    set -e
...
if [ -n "$PIE_EXTENSION" ]; then
    ...
    if which pie; then
      ...
      curl https://github.com/php/pie/releases/latest/download/pie.phar -L -o /usr/local/bin/pie
